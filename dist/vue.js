...
(function (global, factory) {
  ...
  methodsToPatch.forEach(function (method) {
    ...
    // 调用Object.defineProperty,重新定义和修改数组的 原型方法
  ...
  // 注册vm的init方法， 初始化vm
  initMixin(Vue);
  // 注册vm的$data/$props/$set/$delete/$watch方法
  stateMixin(Vue);
  // 初始化事件相关的方法，$on/$once/$off/$emit
  eventsMixin(Vue);
  // 初始化生命周期相关的混入方法， _update/$fourceUpdate/$destroy
  lifecycleMixin(Vue);
  ...
  // 注册全局组件，v-model, v-show， transition，transitionGroup组件
  ...
  Vue.prototype.$mount = function (
    el,
    // 非ssr情况下为false， ssr情况下为true
    hydrating
  ) {
    // 获取el对象，如果el不是dom并且不能通过el查找到对应的dom，创建一个div元素并返回
    el = el && query(el);
    ...
    // 如果没有传render，把template/el转换成render函数
    if (!options.render) {
      var template = options.template;
      // 如果模板存在
      if (template) {
        if (typeof template === 'string') {
          // 如果模板是ID选择器，查找id对应的dom并返回innerHTML
          if (template.charAt(0) === '#') {
            template = idToTemplate(template);
            /* istanbul ignore if */
            // 如果没有template或者template为空，告警
            if ( !template) {
              warn(
